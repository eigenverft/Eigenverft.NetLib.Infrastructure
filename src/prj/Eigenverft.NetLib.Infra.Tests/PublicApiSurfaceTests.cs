...
public sealed class PublicApiSurfaceTests
{
    ...
    public void DeveloperFacingEntryPointsArePublic()
    {
        Type[] publicTypes =
        {
            ...
            typeof(SelfHttpWarmupOptions),
            typeof(SelfHttpWarmupServiceCollectionExtensions),
            typeof(AspNetDataProtectionStringTransforms),
            typeof(AspNetDataProtectionConfigurationValueCodecs),
        };
    ...
    {
        (Assembly Assembly, string TypeName)[] internalTypes =
        {
            (typeof(ConfigurationSetDefinition).Assembly,
            ...
            (typeof(ISwitchableJsonConfiguration).Assembly,
            ...
            (typeof(ConfigurationValueCodec).Assembly,
                "Eigenverft.NetLib.Infrastructure.Hosting.Configuration.Values.ConfigurationValueFormat"),
        };

        foreach ((Assembly assembly, string typeName) in internalTypes)
        {
            Type? type = assembly.GetType(typeName, throwOnError: false);
            Assert.IsNotNull(type, $"Expected implementation type '{typeName}' was not found in '{assembly.GetName().Name}'.");
