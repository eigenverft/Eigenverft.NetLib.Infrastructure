...
```text
./                         you are here (this readme + Eigenverft.NetLib.Security.Certificates.slnx)
../../prj/Eigenverft.NetLib.Security.Certificates/    packable class library
../../prj/Eigenverft.NetLib.Security.Certificates.Tests/  tests (not packed)
```

Package metadata, license, icon, and release notes live in `src/prj/Eigenverft.NetLib.Security.Certificates/NugetAssets/`.
...
Creates one `.nupkg` in `src/prj/Eigenverft.NetLib.Security.Certificates/bin/Pack/` containing the library for all selected target frameworks. Test and optional benchmark projects are not packed.
...
Writes library output to `src/prj/Eigenverft.NetLib.Security.Certificates/bin/Publish/` for the highest selected target framework. This is a class library, not an executable.
