...
- **File/memory limits and prolonged endpoint outages:** the concrete design pass is documented in `LIMITS-OUTAGE-DESIGN.md`. The current implementation still has time-based retention (`sentRetention` default 1 day, `unsentRetention` default 3 days), batch bounds, and the fixed 16384-event emergency safety cap, but no hard database-byte/total-event limit or emergency byte budget yet. The design pass covers intermittent clients, maximum batch wait, file/event/byte limits, endpoint retry/circuit behavior, poison-event handling, bounded emergency memory, shutdown deadlines, observability, and explicit 1h/12h/1d/7d outage semantics. Normal endpoint outages with a healthy local spool remain a file-spool concern, not an emergency-memory concern.
