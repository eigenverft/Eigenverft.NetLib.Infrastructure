...
```text
./                         you are here (this readme + Eigenverft.NetLib.Configuration.Binding.slnx)
../../prj/Eigenverft.NetLib.Configuration.Binding/    packable class library
../../prj/Eigenverft.NetLib.Configuration.Binding.Tests/  tests (not packed)
```

Package metadata, license, icon, and release notes live in `src/prj/Eigenverft.NetLib.Configuration.Binding/NugetAssets/`.
...
Creates one `.nupkg` in `src/prj/Eigenverft.NetLib.Configuration.Binding/bin/Pack/` containing the library for all selected target frameworks. Test and optional benchmark projects are not packed.
...
Writes library output to `src/prj/Eigenverft.NetLib.Configuration.Binding/bin/Publish/` for the highest selected target framework. This is a class library, not an executable.
