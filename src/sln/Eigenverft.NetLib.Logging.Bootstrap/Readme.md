...
```text
./                         you are here (this readme + Eigenverft.NetLib.Logging.Bootstrap.slnx)
../../prj/Eigenverft.NetLib.Logging.Bootstrap/    packable class library
../../prj/Eigenverft.NetLib.Logging.Bootstrap.Tests/  tests (not packed)
```

Package metadata, license, icon, and release notes live in `src/prj/Eigenverft.NetLib.Logging.Bootstrap/NugetAssets/`.
...
Creates one `.nupkg` in `src/prj/Eigenverft.NetLib.Logging.Bootstrap/bin/Pack/` containing the library for all selected target frameworks. Test and optional benchmark projects are not packed.
...
Writes library output to `src/prj/Eigenverft.NetLib.Logging.Bootstrap/bin/Publish/` for the highest selected target framework. This is a class library, not an executable.
